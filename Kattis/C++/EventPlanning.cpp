...
{
	fast
	int participant, budget, hotels, weeks;
	
	cin >> participant >> budget >> hotels >> weeks;

	int currentCost = budget + 1;
	REP(i, 0, hotels)
	{
		int price;
		cin >> price;
		REP(j, 0, weeks)
		{
			...
			if (num >= participant)
				currentCost = min(currentCost, price * participant);
		}
	}
	
	if (currentCost == budget + 1)
		cout << "stay home\n";
	else
